...
def main() -> None:
    ...
    #
    # Окно двое суток, а не одни. Запуск по расписанию плавает: GitHub держит
    # его в очереди, и с 15 сентября 2026 задержка доходила до пяти часов.
    # Окно ровно в сутки при таком плавании оставляет дыры: если вчерашний
    # прогон был в 14:07, а сегодняшний в 15:30, посты за час между ними
    # не увидит ни один. Повторов от лишних суток не будет, их снимает
    # хранилище по id и отпечатку.
    days = int(os.environ.get("DIGEST_DAYS") or 2)
    # Потоки идут по разному расписанию: основной в 09:13, подработка в 10:13.
